...
# Function to get effective tree (from fig5)
get_effective_tree <- function(tree,
                              threshold = 0.05,
                              verbose = FALSE) {
  
  # Compute patristic distances between tips
  dist_mat <- cophenetic(tree)
  
  # -- Cluster tips by threshold --
  # Use UPGMA (average linkage) on the distance matrix
  hc <- hclust(as.dist(dist_mat), method = "average")
  hc$height <- round(hc$height, 6)
  
  clusters <- cutree(hc, h = threshold)
  
  # -- Compute Hill's D1 (effective # of independent clusters) --
  tab <- table(clusters)
  p <- tab / sum(tab)
  D1 <- exp(-sum(p * log(p)))
  if (verbose == TRUE) {
    cat("Effective sequence count (Hill's D1) after clustering:", D1, "")
  }
  
  # -- Collapse tree by keeping one representative per cluster --
  # Select the first tip in each cluster as representative
  reps <- tapply(names(clusters), clusters, `[`, 1)
  tips_to_keep <- unname(reps)
  # Drop all other tips
  drop <- setdiff(tree$tip.label, tips_to_keep)
  collapsed_tree <- drop.tip(tree, drop)
  
  # Return
  l <- list(collapsed_tree = collapsed_tree,
           n_collapsed_branches = length(collapsed_tree$tip.label),
           n_total_branches = length(tree$tip.label),
           ratio = length(collapsed_tree$tip.label) / length(tree$tip.label))
  return(l)
}